...
from modules.gilette_method import configure_algorithm, save_optimization_results, run_gillette_method_block_parallel
...
def main():
    ...
    optimization_lock,optimization_state,simulation_results,best_params_reshape = configure_algorithm(params)
    

    
    if params['optim_type'] == "gilette_method":
        run_gillette_method_block_parallel(params, normalized_target_signal, optimization_lock, 
        ...
        if params['optim_type']=="gilette_method":
            ...
            part_error, error , ECG_original_adaptado  = L2_norm(ecg_path,params['target_ecg_path'])
            
            # Convertir Manager.list a lista normal y filtrar valores None
            all_signals = [x for x in list(optimization_state['all_simulated_signals_original']) if x is not None]
            
        
            graficar_best(all_signals, ECG_original_adaptado, target_signal,error, final='yes')
